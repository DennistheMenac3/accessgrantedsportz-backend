...
export const execute = async (
  ...
  try {
    ...
    // Get week from option or default to latest
    let weekNum = interaction.options.getInteger('week');

    if (!weekNum) {
      // Get latest week with games
    ...
    let response = `🏈 **Week ${weekNum} Scores** | ${league.name}\n`;
    response += `Season ${league.current_season}\n`;
    response += `━━━━━━━━━━━━━━━━━━━━━━\n\n`;
    ...
    gamesResult.rows.forEach((game: any) => {
      ...
      response +=
        `${awayWon ? '🏆 ' : ''}` +
        `**${game.away_abbr}** (${game.away_wins}-${game.away_losses}) ` +
        `**${game.away_score}** - ` +
        `**${game.home_score}** ` +
        `**${game.home_abbr}** (${game.home_wins}-${game.home_losses})` +
        `${homeWon ? ' 🏆' : ''}` +
        `${tie ? ' 🤝' : ''}\n`;
    ...
    try {
      await interaction.editReply('❌ Error fetching scores.');
    } catch {
      // Ignore
    }
  }
