...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption(o => o
    ...
    .setDescription('Picks you want in return — e.g. "1,3"')
    .setRequired(false)
  );
...
export const execute = async (
  ...
  try {
    ...
    // Verify offered players belong to proposer
    ...
    const partnerResult = await query(
      `SELECT t.*, u.discord_user_id as owner_discord_id,
    ...
    const enrichPlayer = async (p: any) => {
      const avResult = await calculateTradeValue(
        p.id, league.id, league.current_season
      );
      return { ...p, av: avResult.total_value };
    };
    ...
    const offerStr = [
      ...enrichedOffered.map((p: any) =>
        ...
        `(${p.position} | ${p.overall_rating} OVR | AV: ${p.av.toFixed(0)})`
      ),
    ...
    await query(
      `INSERT INTO trades (
        ...
        status,
        total_offered_av, total_requested_av,
        created_at
      )
    ...
    // Trade proposal embed for commish channel
    const tradeEmbed = new EmbedBuilder()
      ...
      .setFooter({
        text: `Trade ID: ${tradeId}  ·  AccessGrantedSportz`
      })
      .setTimestamp();
    ...
    await interaction.editReply({
      embeds: [createEmbed(COLORS.SUCCESS)
        ...
        .setDescription(
          `Your trade has been sent for commissioner review.\n\n` +
          `**You send:**\n${offerStr}\n\n` +
          `**You receive:**\n${requestStr}\n\n` +
          `Trade ID: \`${tradeId}\``
        )]
